...
# Define the desired number of rows for each label
num_rows = 10000

# Oversample the minority classes in the training data
train_df_resampled = pd.DataFrame(columns=[DATA_COLUMN, LABEL_COLUMN])
for label in label_list_HARD:
    df_label = train_HARD[train_HARD[LABEL_COLUMN] == label]
    if len(df_label) < num_rows:
        df_label_resampled = resample(df_label, replace=True, n_samples=num_rows, random_state=42)
    else:
        df_label_resampled = df_label.sample(n=num_rows, random_state=42)
    train_df_resampled = pd.concat([train_df_resampled, df_label_resampled])

# Use the resampled training data for the rest of the code
train_HARD = train_df_resampled
...
def model_init():
    ...
    config.dropout = 0.3
    config.weight_decay = 0.01  # Add weight decay regularization
...
training_args.dropout_rate = 0.5  # Increase the dropout rate
...
trainer.train()
torch.save(ns.state_dict(), 'proj.pth')